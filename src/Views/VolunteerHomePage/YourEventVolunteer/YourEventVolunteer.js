...
const YourEventVolunteer = ({shiftId, eventId, userId, shift, isArchived, isReserve}) => {
    ...
    return (
        ...
        <div className='your_event_volunteer'>
            ...
            {!isArchived && !isReserve && <Button type="button" size="xl" style={{color: 'white'}} onClick={() => setConfirmLeave(true)} id="your_event_volunteer_event_sign_off_button"> {t('leaveShift')} </Button>}         
            {!isArchived && isReserve && <Button type="button" size="xl" style={{color: 'white'}} onClick={() => setConfirmLeave(true)} id="your_event_volunteer_event_sign_off_button"> {t('leaveShift')} </Button>}         
